...
def format_datetime_for_api(date_input, end_of_day: bool = False) -> str:
    ...
    a window pass ``end_of_day`` so the day itself falls inside it, instead of
...
def _filter_csv_by_date_range(csv_data: str, start_date: str, end_date: str) -> str:
    ...
    # TIME_SERIES_DAILY_ADJUSTED returns the full series through today. This
    # trim is therefore the only upper-bound guard for a historical request.
    # A parse failure must propagate so the router can try another provider;
    # returning the original body would leak rows after ``end_date``.
