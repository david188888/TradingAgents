...
def get_fundamentals(ticker: str, curr_date: str = None) -> str:
    ...
    ``OVERVIEW`` serves only present-day values and carries no historical
    vintage, so a past ``curr_date`` withholds it through the shared
    point-in-time guard (``date_window.withhold_live_profile``) rather than
    leaking post-decision figures into a historical analysis. The statement
    endpoints below stay point-in-time via ``_filter_reports_by_date``.
