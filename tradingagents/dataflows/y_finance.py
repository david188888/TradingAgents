...
def get_insider_transactions(
    ...
    try:
        ...
        if curr_date:
            ...
            filed = pd.to_datetime(data["Start Date"], errors="coerce")
            valid_filed = filed.notna()
            if not valid_filed.any():
                return (
                    f"<insider transactions unavailable for {canonical} as of {curr_date}: "
                    "Yahoo did not provide parseable filing dates>"
                )
            kept = data.loc[valid_filed & (filed <= cutoff)]
            if kept.empty:
                coverage_start = filed[valid_filed].min().strftime("%Y-%m-%d")
                return (
                    f"<insider transactions unavailable for {canonical} as of {curr_date}: "
                    f"Yahoo serves recent filings only (coverage starts {coverage_start})>"
                )
            data = kept
