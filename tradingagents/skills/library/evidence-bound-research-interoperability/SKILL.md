...
description: Route a company research question to an existing run, research package, reader projection, evidence anchors, and public conversation history without inventing data or issuing trading instructions.
roles:
  - research_manager
triggers:
  - a user asks to analyze a company, ticker, metric, peer comparison, logic edge, claim, evidence, or prior question
  - an Agent needs to resume a run from a manifest, research package, reader projection, or conversation JSONL export
output_schema:
  - company_name
  - ticker
  - question
...
2. For the selected run, load `GET /api/runs/{run_id}/reader/package`.
   ...
   Record `package_sha256` as the canonical hash of the package JSON. In a
   portable export, first verify every `manifest.json.files[*].sha256` and then
   verify `manifest.json.package_sha256` against `research-package.json`.
...
5. Conversation history is public and append-only. Use the HTTP thread API for the selected run:
   - `POST /api/runs/{run_id}/threads` with the package hash and confirmed ticker;
   - `GET /api/runs/{run_id}/threads` to list history;
   - `GET /api/runs/{run_id}/threads/{thread_id}` to read a thread;
   - `POST /api/runs/{run_id}/threads/{thread_id}/messages` to ask a public question;
   - `GET /api/runs/{run_id}/threads/{thread_id}/export` for the verified ZIP bundle.
   The local append-only files under
   `~/.tradingagents/web/runs/{run_id}/conversations/thread_*.jsonl` are the
   storage implementation, not a second lookup authority. Never scan a
   different run's thread file. If no thread is present, history is
   `unavailable`, not evidence that the answer is false.
...
- thread/message: `thread_id` plus contiguous message `sequence` from the thread API response or verified `conversation.jsonl`
...
historical answer must cite the thread message sequence or `thesis_diff` item
as well as the current package anchor.
...
as whether the user wants metrics, peers, logic edges, claims, evidence, or
conversation history. Do not ask for information already present in the
selected run/package.

Return `availability: unavailable` when the run, package, requested anchor,
thread, or provider-backed value is missing, invalid, cross-run, or explicitly
marked unavailable. Include the stable reason (`research_package_unavailable`,
`anchor_not_found`, `thread_unavailable`, etc.) and the next public validation
step. Do not substitute a different company, run, period, or metric. Preserve
package unknowns and observation/comparison unavailable reasons verbatim.
...
Conversation history is an append-only public artifact. Never persist or export
