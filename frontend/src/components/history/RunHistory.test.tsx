...
describe("RunHistory", () => {
  ...
  it("renders only the 3 most recent failed runs", () => {
    ...
    render(<RunHistory runs={manyFailed} loading={false} error={null} onDeleteRun={vi.fn()} onClearHistory={vi.fn()} />);
  ...
  it("renders the placeholder when there are no runs", () => {
    render(<RunHistory runs={[]} loading={false} error={null} onDeleteRun={vi.fn()} onClearHistory={vi.fn()} />);
  ...
  it("renders the loading placeholder while loading with no runs yet", () => {
    render(<RunHistory runs={[]} loading={true} error={null} onDeleteRun={vi.fn()} onClearHistory={vi.fn()} />);
  ...
  it("calls onDeleteRun after confirming a delete button", () => {
    ...
    render(
      <RunHistory runs={FIXTURES} loading={false} error={null} onDeleteRun={onDeleteRun} onClearHistory={vi.fn()} />,
    );
  ...
  it("renders a clear-history button only when runs exist", () => {
    const { unmount } = render(
      <RunHistory runs={FIXTURES} loading={false} error={null} onDeleteRun={vi.fn()} onClearHistory={vi.fn()} />,
    );
    expect(screen.getByRole("button", { name: "清空历史" })).toBeInTheDocument();

    unmount();
    render(
      <RunHistory runs={[]} loading={false} error={null} onDeleteRun={vi.fn()} onClearHistory={vi.fn()} />,
    );
    expect(screen.queryByRole("button", { name: "清空历史" })).not.toBeInTheDocument();
  });

  it("calls onClearHistory after the user confirms", () => {
    const onClearHistory = vi.fn();
    const confirmSpy = vi.spyOn(window, "confirm").mockReturnValue(true);
    render(
      <RunHistory runs={FIXTURES} loading={false} error={null} onDeleteRun={vi.fn()} onClearHistory={onClearHistory} />,
    );
  ...
  it("does not call onClearHistory when the clear is cancelled", () => {
    const onClearHistory = vi.fn();
    const confirmSpy = vi.spyOn(window, "confirm").mockReturnValue(false);
    render(
      <RunHistory runs={FIXTURES} loading={false} error={null} onDeleteRun={vi.fn()} onClearHistory={onClearHistory} />,
  ...
  it("disables the clear button while a clear is in flight", () => {
    render(
      <RunHistory
        runs={FIXTURES}
        loading={false}
