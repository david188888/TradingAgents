...
Covers two systematic fixes:
  ...
  - route_to_vendor emits a definitive no-data sentinel only when every
    required observation authoritatively reports non-coverage.
"""
...
class TestRouteToVendorSentinel(unittest.TestCase):
    ...
    def test_unavailable_fallback_prevents_definitive_no_data(self):
        # One vendor's no-data response plus an unobserved fallback cannot
        # prove that the instrument has no data across the required sources.
        def raises_no_data(symbol, *a, **k):
        ...
        with mock.patch.dict(
            interface.VENDOR_METHODS, {"get_stock_data": patched}, clear=False
        ), self.assertRaises(interface.DataUnavailableError):
