...
def test_reddit_historical_window_excludes_recent(monkeypatch):
    ...
    # The fetched posts only postdate the window, so the source cannot answer
    # for it; "no posts" there would be a claim about the market, not the feed.
    assert "unavailable" in out and "not an absence" in out
...
def test_reddit_covered_but_empty_window_is_a_real_absence(monkeypatch):
    posts = [
        {"title": "NOW", "created_utc": _epoch("2026-08-30"), "source": "rss"},
