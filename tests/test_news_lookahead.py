...
def test_global_news_empty_after_filter_is_informative(monkeypatch):
    ...
    # "no global news" would claim an absence nobody observed.
    assert "unavailable" in out and "not an absence" in out


# --- the article budget counts what the run may read ------------------------


@pytest.mark.unit
def test_out_of_window_articles_do_not_consume_the_article_budget(monkeypatch):
    """The limit counts articles the run may see, not candidates fetched.
    ...
    """
    stale = [
        {"title": f"OLD {i}", "publisher": "P", "link": "l",
         "providerPublishTime": _epoch("2025-01-01")}
        for i in range(2)
    ]
...
def test_the_article_limit_still_caps_what_is_returned(monkeypatch):
    articles = [
        {"title": f"NEWS {i}", "publisher": "P", "link": "l",
         "providerPublishTime": _epoch("2025-05-08")}
        for i in range(5)
    ]
...
# --- coverage vs absence ----------------------------------------------------
# These feeds serve only recent items. When everything fetched postdates the
# window the source cannot answer for that date, so "no news" there would be a
...
def test_ticker_news_window_before_feed_coverage_is_unavailable(monkeypatch):
    recent = [
        {"title": "RECENT", "publisher": "P", "link": "l",
...
def test_global_news_does_not_infer_coverage_from_a_stale_search_hit(monkeypatch):
    ...
    stale = {"title": "STALE", "publisher": "P", "link": "l",
             "providerPublishTime": _epoch("2025-01-01")}
    fresh = {"title": "FRESH", "publisher": "P", "link": "l",
             "providerPublishTime": _epoch("2025-06-01")}
...
def test_coverage_gap_boundaries(dates, expect_gap):
    ...
    if expect_gap:
        assert "unavailable for 2026-05-01..2026-05-08" in out
