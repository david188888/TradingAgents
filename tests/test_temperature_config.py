...
class TestTemperatureForwarding:
    @pytest.mark.parametrize(
        ...
        [
            ...
            # DeepSeek now defaults to thinking mode (which ignores
            # temperature); opt out explicitly so temperature forwarding on
            # the non-thinking path is still exercised.
            ("deepseek", "deepseek-v4-flash", {"thinking": {"type": "disabled"}}),
        ],
    )
    def test_temperature_reaches_client_when_set(self, provider, model, kwargs):
        llm = create_llm_client(
            provider=provider, model=model, temperature=0.0, api_key="placeholder", **kwargs
        ).get_llm()
...
class TestProviderKwargsTemperature:
    """provider_llm_kwargs float-coerces and forwards temperature, or omits it."""

    def _kwargs_for(self, temperature):
        from tradingagents.llm_clients.provider_kwargs import provider_llm_kwargs

        return provider_llm_kwargs({"llm_provider": "openai", "temperature": temperature})
